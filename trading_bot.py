...
def train_ensemble(train_data, mapping, valid_symbols, timestamps, params):
    logging.info('Started training ensemble...')
    ...
    models = []
    for i in range(3):  # Example: train 3 models
        train_env = DummyVecEnv([lambda: CryptoTradingEnv(train_data, mapping, valid_symbols, timestamps, params)])
        train_env = VecNormalize(train_env, norm_obs=True, norm_reward=True)
    ...
    for idx, model in enumerate(models):
        model.save(os.path.join(output_dir, f"model_ppo_crypto_trading_{idx}"))

    # Restore the original stdout
    sys.stdout.log.close()
    sys.stdout = original_stdout

    return models, output_dir

def test_ensemble(models, test_data, mapping, timestamps, valid_symbols, output_dir, params, repetition=None):
    # Redirect stdout to both console and logfile
    logfile_path = os.path.join(output_dir, "trading_bot_log.txt")
    sys.stdout = DualOutput(logfile_path)
    
    logging.info('Started testing ensemble...')
    ...
    # Run the ensemble of models on the test environment
    obs = test_env.reset()
    done = False
    while not done:
        ...
        # Use majority voting for discrete actions
        action = max(set(actions_as_tuples), key=actions_as_tuples.count)
        # Convert the selected action back to a numpy array
