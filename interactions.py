...
def compute_prices(data, action, tp_percentage, sl_percentage):
...
import pandas as pd
import logging
# from interactions import fetch_symbols, open_trade
...
class TestExecuteTrade(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.data = {}
        self.data[self.symbol] = fetch_binance_klines(self.symbol + 'USDT', self.interval, 100)
    ...
    def test_close_all_open_trades(self):
        try:
            ...
            for symbol, trade_index, is_long in open_trades:
                ...
                data = fetch_binance_klines(symbol + 'USDT', '1m', 100)
    ...
    def test_open_trade_minimal_collateral(self):
        action = 'open_long'
        leverage = 50
        tp_price, sl_price = compute_prices(self.data[self.symbol], action, 0.02, 0.01)
        open_trade(self.data[self.symbol], self.pairs, self.symbol, action, 50, leverage, tp_price, sl_price)
        df = pd.DataFrame(self.data[self.symbol])
...
# 2024-10-02 13:18:14,888 - INFO - Executed trade for NEAR with type open_long and info {'type': 'open_long', 'collateral': 198.49247999999997, 'leverage': 8, 'tp_price': 4.878717928369065, 'sl_price': 4.820641035815467}
        ...
        leverage = 8
        tp_price, sl_price = compute_prices(self.data[self.symbol], action, 0.02, 0.01)
        # tp_price = 4.878717928369065
        # sl_price = 4.820641035815467
        open_trade(self.data[self.symbol], self.pairs, self.symbol, action, collateral, leverage, tp_price, sl_price)
        df = pd.DataFrame(self.data[self.symbol])
        logging.info(df.tail())
    ...
    def test_close_latest_trade(self):
        try:
            ...
            # Fetch the latest price for the symbol
            data = fetch_binance_klines(symbol + 'USDT', '1m', 100)  # Assuming fetch_binance_klines returns the latest kline data
            latest_price = data.iloc[-1]['close']  # Assuming the data is returned as a DataFrame
