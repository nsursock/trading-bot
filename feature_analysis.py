...
from trading_bot import create_financial_params, prepare
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(f"Data prepared. Shape: {data.shape}, Symbols: {len(valid_symbols)}")
        
        if data.shape[0] != financial_params['limit']:
            logger.info(f"Data shape does not match limit. Expected {financial_params['limit']}, got {data.shape[0]}")
            sys.exit(1)
        
        # Calculate future returns (target variable)
        logger.info("Calculating future returns")
        y = calculate_future_returns(data, mapping)
        logger.info(f"Future returns calculated. Shape: {y.shape}")

        # Prepare features (X)
        logger.info("Preparing features")
        num_candles, num_symbols, num_features = data.shape
        X = data[:-1].reshape(num_candles - 1, num_symbols * num_features)  # Remove last row to align with y
        ...
        if args.model:
            ...
            env = DummyVecEnv([lambda: CryptoTradingEnv(data, mapping, valid_symbols, timestamps, financial_params)])
            ...
            feature_importance = calculate_feature_importance(X_scaled, y, model, env, feature_names, method=args.method)
        ...
        # # Save overall feature importance plot
        # logger.info(f"Generating overall {args.method} feature importance plot")
        # fig, ax = plt.subplots(figsize=(20, 10))
        # plt.bar(range(len(feature_importance)), feature_importance)
        # plt.title(f"{args.method.capitalize()} Feature Importance (Overall)")
        # plt.xlabel("Feature")
        # plt.ylabel("Importance")
        # plt.xticks(range(len(feature_names)), feature_names, rotation=90)
        # plt.tight_layout()
        # save_plot(fig, f"{args.method}_feature_importance_overall.png")
        # logger.info(f"Overall {args.method.capitalize()} feature importance plot saved as {args.method}_feature_importance_overall.png")
        ...
        # logger.info("Analyzing a specific instance")
        # X_train, X_test, y_train, y_test = train_test_split(X_scaled, y, test_size=0.2, random_state=42)
        
        # fig = shap.force_plot(explainer.expected_value, shap_values[0], X_test[0], feature_names=feature_names, show=False, matplotlib=True)
        # save_plot(fig, "shap_force_plot.png")
        # logger.info("SHAP force plot for a specific instance saved as shap_force_plot.png")
