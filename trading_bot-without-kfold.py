...
def test(model, test_data, mapping, timestamps, valid_symbols, output_dir, params, repetition=None):
    ...
    # Calculate performance score as strategy return TODO understand discrepancy
    # current_balance = test_env.envs[0].balance
    # performance_score = (current_balance - initial_balance) / initial_balance * 100
    # performance_score = round(performance_score, 2)

    # print(f"Test Performance Score (Strategy Return 1): {performance_score}%")
    ...
    # trade_metrics = get_trade_metrics(test_env.envs[0].trade_history, test_env.envs[0].symbols)
    # net_profit = trade_metrics.get('PF', {}).get('net_profit', 0)
    # sharpe_ratio = trade_metrics.get('PF', {}).get('sharpe_ratio', 0)
    # max_drawdown = trade_metrics.get('PF', {}).get('max_drawdown', 0)

    # # Combine metrics into a performance score
    # performance_score = (net_profit / params['initial_balance'] * 100) + (sharpe_ratio * 10) - (max_drawdown * 100)
    # performance_score = round(performance_score, 2)

    # print(f"Test Performance Score (Combined): {performance_score}%")
...
def main():
    ...
    if data is not None:
        ...
        # Example usage
        model, output_dir = train(train_data, mapping, valid_symbols, timestamps[:split_index], financial_params)
        if model:
            test_performance_score = test(model, test_data, mapping, timestamps[split_index:], valid_symbols, output_dir, financial_params)
